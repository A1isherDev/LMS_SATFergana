...
app.conf.beat_schedule = {
    ...
    'update-monthly-rankings': {
        ...
        'schedule': crontab(minute='0', hour='1', day_of_month='1'),  # 1st of month 1 AM
    },
    ...
    'cleanup-old-rankings': {
        ...
        'schedule': crontab(minute='0', hour='0', day_of_month='1'),  # 1st of month midnight
    },
...
# We need to load settings to get REDIS_URL
# Since app.config_from_object is lazy, we can't access settings yet if we don't import them carefully
# But settings.REDIS_URL is used here. 
# It's better to rely on config_from_object for standard keys and set custom ones if needed, 
# or just assume settings are loaded.
# Checking celery_config.py again... it used settings.REDIS_URL directly.

# Re-import settings to be safe or rely on django.conf
# from django.conf import settings is already there.

app.conf.broker_url = getattr(settings, 'REDIS_URL', 'redis://localhost:6379/0')
